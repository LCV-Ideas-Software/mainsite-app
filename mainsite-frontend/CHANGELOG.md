...
- **Tightening de `style` por allowlist de VALOR (issue #411)**: novo
  `lib/inline-style-allowlist.ts` aplicado no pós-processamento DOM dos dois
  call sites DOMPurify (PostReader e AboutPage). Em vez de remover o atributo
  `style` (o que regrediria o conteúdo TipTap publicado — rationale do
  diferimento na v03.22.00), cada `style` é reescrito mantendo apenas as
  propriedades que o editor emite (text-align, text-indent, font-family,
  font-size, color, width, margens) com o valor validado; `url()`,
  `expression()`, `position` e afins caem. O CSSOM nativo faz o parse
  (shorthands expandem em longhands), sem parsing manual de string. Fecha o
  diferimento registrado na v03.22.00.

### Validação

- 6 testes novos do helper (round-trip completo do conjunto TipTap, shorthand
  margin, remoção de propriedades/valores maliciosos) + caso de integração no
  AboutPage; RED provado contra o código anterior (style malicioso sobrevivia).
