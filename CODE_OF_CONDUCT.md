# Code of Conduct

This project adopts the **Contributor Covenant 3.0** as its community standard. The full canonical text is available at:

- https://www.contributor-covenant.org/version/3/0/code_of_conduct/
- Plain-text mirror: https://www.contributor-covenant.org/version/3/0/code_of_conduct/code_of_conduct.md

By participating in this project (contributing code, filing issues, opening pull requests, participating in discussions, or operating a deployment of this app), you agree to follow the Contributor Covenant 3.0 standard linked above.

---

## Scope

This applies to:

- All project spaces on GitHub (issues, pull requests, discussions, commits, releases).
- Any public representation of the project, including conference talks, blog posts, and social media posts identifying as maintainers or contributors.

---

## Enforcement contact

Reports of violations can be sent privately to:

**conductcode@lcv.dev**

This channel is only for conduct reports. Security vulnerabilities must follow [SECURITY.md](./SECURITY.md) and be reported to **security@lcv.dev**. Conduct reports will be acknowledged within 48 hours and handled per the Contributor Covenant 3.0 enforcement ladder.

All reporters will have their identity kept confidential.

---

## Enforcement actions

Per Contributor Covenant 3.0, the maintainer reserves the right to apply any of the following actions in response to a violation:

1. **Correction** — private warning + clarification.
2. **Warning** — formal written warning + temporary restriction on participation.
3. **Temporary ban** — time-bounded removal from project spaces.
4. **Permanent ban** — permanent removal.

Decisions are at the maintainer's discretion guided by the Contributor Covenant 3.0 enforcement ladder.
